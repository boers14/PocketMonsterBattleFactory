...
public class PhysicalPreperations : PocketMonsterAbility
{
    public override void SetAbilityStats(PlayerBattle player)
    {
        abilityName = "Physical Preperations";
        oneTime = true;
        instantEffect = true;
        abilityDescription = "Boosts the attack stat of the next pocketmonster in the party by two stages. Will fail if the next pocketmonster is" +
    ...
    public override void UseInstantAbility(PocketMonster ownPocketMonster, PocketMonster opponentPocketMonster, InBattleTextManager inBattleTextManager)
    {
        ...
        {
            trainedPocketmonster.stats.attack.GetStatChanges(2);
